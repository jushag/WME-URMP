...
//    WMEURMPT.newDataAvailable(0)
  ...
  WMEURMPT.getMPs = function (bounds, filter) {
    const url = 'https://' + document.location.host + WMEURMPT.wazeConfigApiFeatures + '?language=en' + (WMEURMPT.scanUR ? '&mapUpdateRequestFilter=3%2C0' : '') + (WMEURMPT.scanMP ? '&problemFilter=3%2C3' : '') + '&mapComments=' + (WMEURMPT.scanMC ? 'true' : 'false') + (WMEURMPT.scanPUR ? '&venueLevel=4&venueFilter=1%2C1%2C1%2C0' : '') + '&editableAreas=true&bbox=' + bounds[0] + '%2C' + bounds[1] + '%2C' + bounds[2] + '%2C' + bounds[3]
    let xhr3Object = null
    if (XMLHttpRequest) {
      xhr3Object = new XMLHttpRequest()
    } else if (ActiveXObject) {
